...
WANDB_ENTITY = None 
WANDB_RUN_NAME = "unet_char_emb_v2" 
...
class CharacterTokenEncoder(nn.Module):
    def __init__(self, alphabet_str, emb_dim, rnn_hidden_dim, rnn_layers,
                 target_feature_width): # target_feature_width - это W_base_text
        super().__init__()
        ...
        self.rnn_output_dim = rnn_hidden_dim * 2 # Это будет text_channels_in для SpatialFiLM
        self.target_feature_width = target_feature_width

        self.adaptive_pool = nn.AdaptiveAvgPool1d(target_feature_width)
        print(f"CharacterTokenEncoder (Spatial Features): Vocab Size: {self.vocab_size}, Emb Dim: {emb_dim}")
        print(f"  RNN: Hidden Dim (per direction): {rnn_hidden_dim}, Layers: {rnn_layers}")
        print(f"  Output RNN Dim (TextChannels): {self.rnn_output_dim}")
        print(f"  Target Output Feature Width (W_base_text): {target_feature_width}")
    ...
    def forward(self, texts_batch, max_len_chars_for_tokenization=60):
        ...
        # (B, MaxLenChars, RnnOutputDim) -> (B, RnnOutputDim, MaxLenChars)
        ...
        # Добавляем высоту = 1: (B, RnnOutputDim, 1, target_feature_width)
        spatial_text_features_base = text_features_pooled.unsqueeze(2)
        
        return spatial_text_features_base
...
class VAEEncoderWithSkips(nn.Module):
    # ... (полное определение как в вашем paste.txt) ...
    def __init__(self, in_ch=4, z_ch=Z_CH):
        super().__init__()
        self.e_conv1 = self._conv_block(in_ch, 64)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.e_conv2 = self._conv_block(64, 128)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.e_conv3 = self._conv_block(128, 256)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.e_conv4 = self._conv_block(256, 512)
        self.pool4 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bottleneck_conv = self._conv_block(512, 1024)
        
        self.feature_map_h = PATCH_SHAPE[1] // 16
        self.feature_map_w = PATCH_SHAPE[0] // 16
        self.mu_head = nn.Conv2d(1024, z_ch, kernel_size=(self.feature_map_h, self.feature_map_w))
        self.logvar_head = nn.Conv2d(1024, z_ch, kernel_size=(self.feature_map_h, self.feature_map_w))
    ...
    def forward(self, x):
        s1_out = self.e_conv1(x); p1_out = self.pool1(s1_out)
        s2_out = self.e_conv2(p1_out); p2_out = self.pool2(s2_out)
        s3_out = self.e_conv3(p2_out); p3_out = self.pool3(s3_out)
        s4_out = self.e_conv4(p3_out); p4_out = self.pool4(s4_out)
        ...
        return mu, logvar, [s1_out, s2_out, s3_out, s4_out]
...
class VAEDecoderWithSpatialFiLM(nn.Module):
    def __init__(self, z_ch, text_channels_in, out_ch_image, patch_h, patch_w):
        super().__init__()
        self.initial_h = patch_h // 16
        self.initial_w = patch_w // 16
        skip_channels_from_encoder = [512, 256, 128, 64] # s4, s3, s2, s1

        self.bottleneck_proc = nn.Sequential( 
            nn.ConvTranspose2d(z_ch + text_channels_in, 1024, 
                               kernel_size=(self.initial_h, 1), # Из (1, W_base) в (H_base, W_base)
                               stride=1, padding=0),             # (B, 1024, H/16, W/16)
            nn.BatchNorm2d(1024), 
            nn.ReLU(inplace=True)
        )
        
        # Блоки декодера: upsample -> cat_with_skip -> SpatialFiLM -> convs
        
        # Этап 1: H/16 -> H/8
        self.up_tconv1 = nn.ConvTranspose2d(1024, 512, kernel_size=2, stride=2)
        num_features_after_cat1 = 512 + skip_channels_from_encoder[0] # 512+512=1024
        self.spatial_film1 = SpatialFiLMLayer(text_channels_in, num_features_after_cat1)
        self.conv_block1 = self._conv_block_after_modulation(num_features_after_cat1, 512)

        # Этап 2: H/8 -> H/4
        self.up_tconv2 = nn.ConvTranspose2d(512, 256, kernel_size=2, stride=2)
        num_features_after_cat2 = 256 + skip_channels_from_encoder[1] # 256+256=512
        self.spatial_film2 = SpatialFiLMLayer(text_channels_in, num_features_after_cat2)
        self.conv_block2 = self._conv_block_after_modulation(num_features_after_cat2, 256)

        # Этап 3: H/4 -> H/2
        self.up_tconv3 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        num_features_after_cat3 = 128 + skip_channels_from_encoder[2] # 128+128=256
        self.spatial_film3 = SpatialFiLMLayer(text_channels_in, num_features_after_cat3)
        self.conv_block3 = self._conv_block_after_modulation(num_features_after_cat3, 128)

        # Этап 4: H/2 -> H
        self.up_tconv4 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        num_features_after_cat4 = 64 + skip_channels_from_encoder[3] # 64+64=128
        self.spatial_film4 = SpatialFiLMLayer(text_channels_in, num_features_after_cat4)
        self.conv_block4 = self._conv_block_after_modulation(num_features_after_cat4, 64)
        
        self.final_image_conv = nn.Conv2d(64, out_ch_image, kernel_size=1)
        self.output_activation_fn = nn.Sigmoid()
        print("VAEDecoderWithSpatialFiLM initialized.")

    def _conv_block_after_modulation(self, in_c, out_c, kernel_size=3, padding=1):
        return nn.Sequential(
            nn.Conv2d(in_c, out_c, kernel_size, padding=padding, bias=False),
            nn.BatchNorm2d(out_c), nn.ReLU(inplace=True),
            nn.Conv2d(out_c, out_c, kernel_size, padding=padding, bias=False),
    ...
    def forward(self, z_latents, spatial_text_features_base, skips_list):
        ...
        # spatial_text_features_base: (B, text_channels_in, 1, W_base_text)
        # skips_list: [s1, s2, s3, s4]
        
        # Объединяем z и базовые текстовые признаки для bottleneck_proc
        z_expanded_spatial = z_latents.expand(-1, -1, 1, self.initial_w) # (B, z_ch, 1, W_base)
        combined_bottleneck_input = torch.cat([z_expanded_spatial, spatial_text_features_base], dim=1)
        x_current = self.bottleneck_proc(combined_bottleneck_input) # (B, 1024, H/16, W/16)
        
        # Этап 1
        x_upsampled = self.up_tconv1(x_current)
        x_concat = torch.cat([x_upsampled, skips_list[3]], dim=1) # s4
        ...
        x_concat = torch.cat([x_upsampled, skips_list[2]], dim=1) # s3
        ...
        x_concat = torch.cat([x_upsampled, skips_list[1]], dim=1) # s2
        x_modulated = self.spatial_film3(x_concat, spatial_text_features_base)
        x_current = self.conv_block3(x_modulated)
        
        # Этап 4
        x_upsampled = self.up_tconv4(x_current)
        x_concat = torch.cat([x_upsampled, skips_list[0]], dim=1) # s1
        x_modulated = self.spatial_film4(x_concat, spatial_text_features_base)
        x_current = self.conv_block4(x_modulated)
